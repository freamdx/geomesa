...
class GeoMesaDataSource extends DataSourceRegister
  ...
  override def createRelation(sqlContext: SQLContext, parameters: Map[String, String]): BaseRelation = {
    ...
    val showFid = Try(parameters("showFid").toBoolean).getOrElse(false)
    val schema = sft2StructType(sft, showFid)
  ...
  def sft2StructType(sft: SimpleFeatureType, showFid: Boolean = true): StructType = {
  ...
  override def createRelation(sqlContext: SQLContext, mode: SaveMode, parameters: Map[String, String], data: DataFrame): BaseRelation = {
    ...
    val showFid = Try(parameters("showFid").toBoolean).getOrElse(false)
    val structType = if (data.queryExecution == null) { sft2StructType(sft, showFid) } else { data.schema }
...
case class GeoMesaRelation(sqlContext: SQLContext,
  ...
  val gridPartition: Boolean = Try(params("gridPartition").toBoolean).getOrElse(false)
  ...
  val geometryOrdinal: Int = if (sft.getGeometryDescriptor == null) {
    -1
  } else {
    sft.indexOf(sft.getGeometryDescriptor.getLocalName)
  }
  ...
  if (partitionedRDD == null && spatiallyPartition) {
    if (partitionEnvelopes == null) {
      ...
      partitionEnvelopes = partitionStrategy match {
        case "EARTH" | "EQUAL" =>
          if (gridPartition) numPartitions = RelationUtils.computePartitions(bounds, numPartitions, gridFactor)
  ...
  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    val struct = data.schema
    val fidFn: Row => String = struct.fields.indexWhere(_.name == "__fid__") match {
      case -1 => _ => TimeSortedUuidGenerator.createUuid().toString
      case i => r => r.getString(i)
    }

    val encodeSft = encodedSFT
    val newFeatureName = params(GEOMESA_SQL_FEATURE)
    val rddToSave: RDD[SimpleFeature] = data.rdd.mapPartitions(iterRow => {
      val sftPar = SimpleFeatureTypes.createType(newFeatureName, encodeSft)
      val mappings = SparkUtils.sftToRowMappings(sftPar, struct)
      iterRow.map(r => SparkUtils.row2Sf(sftPar, mappings, r, fidFn(r)))
    })
    GeoMesaSpark(params).save(rddToSave, params, newFeatureName)
  }
}
